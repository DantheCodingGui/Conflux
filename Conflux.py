...
class Game():
    ...
    def state_manager(self): #controls the flow of the game between possible game states
    ...
    def end_menu(self): #the menu shown at the end of every game
        pygame.mouse.set_visible(1)
        self.player_name = ''
    ...
    def is_high_score(self): #sorts the high scores from low to high at the end of every game session
        self.high_scores.append(Score())
    ...
    def view_high_scores(self):
        ...
        while viewscores:
            ...
            if self.high_scores[0].score == 0:
                ...
                self.message_display(self.high_scores[0].name, self.font, 20, self.colour_code('black'), 425, 350)
                if self.high_scores[0].score == 11:
                    self.message_display('Win!', self.font, 20, self.colour_code('black'), 575, 350)
                else:
                    self.message_display(str(self.high_scores[0].score), self.font, 20, self.colour_code('black'), 575, 350)
                self.message_display(self.high_scores[1].name, self.font, 20, self.colour_code('black'), 425, 400)
                if self.high_scores[1].score == 11:
                    self.message_display('Win!', self.font, 20, self.colour_code('black'), 575, 400) 
                elif self.high_scores[1].score != 0:
                    self.message_display(str(self.high_scores[1].score), self.font, 20, self.colour_code('black'), 575, 400) 
                self.message_display(self.high_scores[2].name, self.font, 20, self.colour_code('black'), 425, 450)
                if self.high_scores[2].score == 11:
                    self.message_display('Win!', self.font, 20, self.colour_code('black'), 575, 450)
                elif self.high_scores[2].score != 0:
                    self.message_display(str(self.high_scores[2].score), self.font, 20, self.colour_code('black'), 575, 450)            
    ...
    def game_icon(self, icon): #loads the game icon
        game_icon = pygame.image.load(icon).convert_alpha()
        pygame.display.set_icon(game_icon)

    def colour_code(self, colour): #defines all of the colours used throughout the program
        #               R     G    B
        cols = {
            'white' :   (255, 255, 255),
            'black' :   (  0,   0,   0),
    ...
    def load_map(self): #loads the map boundary, and the enemies/barriers that are specific to the room the player is currently in
        ...
        if self.room_number == 2:
            ...
            if self.first_run:
                ...
                barrier3 = Barrier(500, 50, 50, 400)
            ...
            if self.first_run:              
                ...
                barrier1 = Barrier(250, 260, 50, 80)
                barrier2 = Barrier(400, 120, 50, 80)
                barrier3 = Barrier(400, 400, 50, 80)
                self.barriers_list.add(barrier1, barrier2, barrier3) 
        elif self.room_number > 10:
            self.game_state = 'End'
               
    def get_uvector(self, x, y, angle): #gets the unit vector for the movement of any object that is is called for
    ...
    def get_edges(self, item, image): #returns the 4 edges of the rectangle around the image it is called for, used for collision detection with the map boundary and barriers
